...
- Research-grounded confidence: for each priority, cite the deep research signal
  strength (Strong/Moderate/Weak) that supports it, if available
...
gh issue create \
  ...
  --body "<full report body including sections 1-7 and the ## Synthesis block, plus a footer: 'Generated by UXR Synth agent run on <YYYY-MM-DD>. Upstream: Deep Research + Ideation.'>" \
  --label "uxr-synthesis"
```
