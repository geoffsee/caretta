...
Research-Derived / Provocations / Wildcards). For each surviving idea: one-sentence
description, one-sentence rationale, research citation where applicable, and (if the
human requested it) a short expansion paragraph.
...
gh issue create \
  ...
  --body "<full ideation set with all buckets and surviving ideas, plus a footer: 'Generated by Research-Informed Ideation agent run on <YYYY-MM-DD>. Upstream: Deep Research.'>" \
  --label "ideation"
```
